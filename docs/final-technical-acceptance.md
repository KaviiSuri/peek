...
The fixed 50ms core character probe remains, including failure diagnostics. It is not a universal latency SLA or natural-idle evidence. Warm/cold workload samples await AX readiness and worker attachment before typing; their fields are explicitly readiness-gated. They cannot establish immediate cold-input preservation. Separate fixed-delay native attempts are now prepared in `earlyCharacters`, but their native execution and early cold-character qualification remain pending. Natural idle requires worker-debugging detach and actual target disappearance without forced termination. The source must remain focused immediately before native posting; a focus-lost attempt is recorded as unmeasured, not silently refocused and called cold. Forced restart and fresh-profile startup remain separately labelled.
...
## Prepared early-character attempt (not executed natively)

Each 30/100-tab warm/cold workload trial first has a separate native character attempt. `native-early-character.swift` posts Control+Space and then attempts Unicode `g` at a scheduled 50ms delay using macOS keycodes 49/5. It uses one monotonic scheduler without waiting for AX readiness or worker reattachment. Native call timestamps and actual dispatch offset are recorded; scheduler delays are not hidden. Key-up/modifier flags are cleared, and a late dispatch still gets a non-collapsed release interval.

Posting requires the already-authorized, executable/profile-verified disposable PID and matching foreground app. The harness records source/palette/worker preconditions and establishes natural worker absence without a forced stop. It observes source/palette state only after the native pair; hits, misses, aborts and unavailable observations remain separate from readiness-gated samples. A late hit is evidence at its actual offset, not a universal 50ms SLA. Evidence is saved before a separately labelled recovery phase; external foreground ownership stops recovery without activation. The existing core 50ms probe is unchanged.

Code-only validation, safe while desktop QA is paused:

```sh
node scripts/check-early-native.mjs /absolute/dry-evidence/path
```

This compiles the shared scheduler, runs only `--dry-run` with a recording sink, and checks three isolated mutations. It exits before permission/application inspection or CGEvent construction/posting. Compilation/dry-run is not native character evidence. The full matrix now adds eight natural-idle waits; plan a longer coordinated desktop interval before native execution.

## All 22 stories

**Automated source checks pass; final Chrome qualification is incomplete.** Production bytes remain at checkpoint `62b1950`; the code-only suite now passes 221 tests across 18 files, typecheck and build. Thirty-one isolated author mutations fail on their intended behavior assertions, including all favicon resource bounds, teardown departures, obsolete return authority, global recovery status, acknowledged-state ABA persistence, and the harness's first-focus/readiness and early-character evidence checks. A separate Swift compile/dry-run audit passes its control and rejects three scheduling/keycode/modifier mutations without posting any events. Parent review remains independent.
