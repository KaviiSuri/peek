...
import {
  ...
  ingestOverlayMarkers,
  startTimelineRun
} from "./instrumentation/worker-timeline.ts"
...
function invoke(tabId: number | undefined): void {
  ...
  // First observable point: worker startup/key-dispatch before this callback is inaccessible.
  startTimelineRun(tabId)
  // Start injection directly from the action gesture; do not await storage or unrelated work first.
  ...
  void (async () => {
    try {
      if (message.type === "peek-centered-overlay:get-model") {
        ingestOverlayMarkers(sourceTabId, message.timeline)
        sendResponse({
          ok: true,
          model: await modelFor(sourceTabId),
          instrumentationRunId: currentTimelineRunId(sourceTabId)
        } satisfies OverlayResponse)
        return
      }
      if (message.type === "peek-centered-overlay:timeline") {
        ingestOverlayMarkers(sourceTabId, message.timeline, message.runId)
