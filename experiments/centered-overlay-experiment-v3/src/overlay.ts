...
import { makeOverlayTimeline } from "./instrumentation/overlay-timeline.ts"
import type { OverlayResponse, WorkerRequest } from "./protocol.ts"

const timeline = makeOverlayTimeline()
timeline.mark("overlay-entry")
...
if (toggleOutcome === "mounted" && host !== null) {
  ...
  void (async () => {
    try {
      timeline.mark("model-request")
      const response = (await chrome.runtime.sendMessage({
        type: "peek-centered-overlay:get-model",
        timeline: timeline.drain()
      })) as OverlayResponse
      timeline.mark("model-response")
      if (!response.ok) throw new Error(response.error)
      ...
      requestAnimationFrame(() => {
        ...
        requestAnimationFrame(() => {
          ...
          void chrome.runtime.sendMessage({
            ...
            runId: response.instrumentationRunId,
            timeline: timeline.drain()
          })
        })
      })
