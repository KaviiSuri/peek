...
The manifest uses `"mac": "MacCtrl+Space"`. In Chromium command syntax, that is physical Control—not `Ctrl`, which maps to Command on macOS. Control+Space is only the candidate Kavii observed working in Dia; it is not a universal or Chrome default.
