...
Peek includes fuzzy title/URL matching, a Tab-toggled keyboard selection mode, and a transient extension-window fallback for restricted pages. Very early input before the palette mounts remains an [open issue](https://github.com/KaviiSuri/peek/issues/8).

## Fuzzy search

Version0.2.0 uses the bundled `fzf` JavaScript matcher with the fzf-style behavior used by Telescope's fzf-native extension:

- Short abbreviations work: `gh` can match `GitHub`; letters must appear in order but may have gaps.
- Consecutive letters and word starts score well. Space-separated terms must all match, in either order, across the title and displayed URL.
- Lowercase queries ignore case. A term containing uppercase letters is case-sensitive.
- Optional fzf syntax includes `'exact`, `!exclude`, `^prefix`, `suffix$`, and `one | another`. Prefix and suffix refer to the whole searchable label: title, a space, then location.
- Matching characters are bolded using fzf's actual match positions.
- Nonempty queries rank by fzf score, shorter label, then original tab order. There are no repository-specific boosts or recency overrides. An empty query still puts your previous tab first, followed by non-current MRU tabs.

Search uses NFC-normalized title and location text. The displayed/searchable location retains host, port, path, query and fragment, but excludes URL scheme and credentials. Accents are not stripped. Fzf does not correct substituted or transposed letters.

Everything runs locally; no remote code or additional permissions. This is a JavaScript port, not a claim of byte-identical native Telescope behavior. See the [source comparison and adoption note](docs/discovery/telescope-fuzzy-matching.md).
